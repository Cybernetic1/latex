...
\title{\vspace{-1.5cm} \bfseries\color{blue}{\LARGE Future News - some ideas}}
...
\begin{itemize}
	\item DAO is the way we organize this project, I propose.
	\item Anyone can make contributions to this project, in accord to the open policy I proposed in 	{\color{red}{\large \textcircled{\small 1}}} for news items.
	\item We can use \textbf{mutual weighted-voting} to determine each members' shares.
	\item I have developed an experimental web GUI for this:
	\begin{equation}
		\vcenter{\hbox{\includegraphics[scale=0.3]{COCO-screenshot-EN.png}}} \nonumber
	\end{equation}
	\item Usually, wealth follows a \textbf{log-normal distribution} (right) in our society: \\
	\begin{equation}
		\vcenter{\hbox{\includegraphics[scale=0.5]{normal-vs-log-normal.png}}} \nonumber
	\end{equation}
	which means most workers are at the \textbf{poor-end} of the spectrum.
	\item In our DAO, we transform the curve to a \textbf{Gaussian} distribution (left).  This means the ``shares'' of our DAO would be more evenly distributed.  Everyone makes more or less the same, so we don't need to fight fiercely about who contributed more.  In return for this, our project gets more successful \textbf{as a whole}, so everyone makes more money.
	
	% \item Talent is distributed \textbf{normally} among people (Gaussian distribution), but human attention has a multiplying effect, leading to \textbf{exponential earnings} of top performers: \\
	% \begin{equation}
