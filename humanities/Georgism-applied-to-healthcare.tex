...
\title{\vspace{-1cm} \bfseries\color{blue}{\huge \cc{Georgism 应用到 全民医保}{Georgism Applied to\\ Universal Healthcare}}}
...
\section{\cc{Henry George 关于地产霸权的思想}{Henry George's ideas on land reform}}

\cc{
``\textbf{Affordable housing}'' 这个术语 可以被翻译成「可负担住房」，但不纯粹是「廉价住房」那么简单。 它更接近于 习近平 所说的「房是用来住，不是用来炒」的意思，简称「\textbf{房住不炒}」。\\ \nocite{潘慧娴2010}
}{
``\textbf{Affordable housing}'' is one of the main problems of social justice.  The traditional ideal of liberalism is for everyone to have the freedom to achieve their goals in life.  But if the basic necessities are deprived from people then they could be in situations that are virtually without freedom.\\

Housing is an acute problem in most developed countries in particular in big cites.  In Hong Kong (where I'm from) the real-estate tycoon families were a ruling class established during the British colonial period, as a means of using locals to control the masses.  The unfair exploitation of housing became a problem in Hong Kong that persists to this day\cite{Poon2011}.  In China, Xi Jinping in 2016 proposed the political slogan, ``\textbf{Houses are for living, not for speculation}.''\\
}
...
But the answer to the housing problem might have been proposed nearly one and a half century ago by the American economist Henry George.	His solution is the \textbf{land tax} \\
}
...
\section{\cc{将 ``Georgism'' 应用到 全民医保}{Georgism applied to universal healthcare}}
