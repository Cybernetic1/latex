...
\begin{enumerate}[label=(\Alph*)]
	\item News site.  Gas fee is a problem -- who would pay even \$10 a year just to be able to ``like'' others' comments on a news site?  The news site has to provide economic value to users, and then gas fees can be deducted from the revenues generated by the news site.  But

	\item A business run by democratic voting -- can be any business, let's assume it's a restaurant.
\end{enumerate}
